...
func (app *application) GetStudentsData(sf mysql.StudentFilter) ([]mysql.Student, error) {
	data, err := app.rediconn.ReadAllData()
	if err != nil {
		if err.Error() == config.RedisErrKeyDoesNotExist {
			err = nil

			data, err = app.sqlconn.GetStudents(sf)
			if err != nil {
				return data, err
			}
...
func (app *application) RegisterStudent(ms mysql.Student) error {
	id, err := app.sqlconn.CreateStudent(ms)
	if err != nil {
		return err
	}
